...
class Robot(object):
    def __init__(self, addr=0x6f, left_id=2, right_id=1, left_trim=0, right_trim=0,
                 stop_at_exit=True):
    ...
    def left(self, speed, seconds=None):
        ...
        self._left_speed(speed)
        self._right_speed(speed)
        self._right.run(Adafruit_MotorHAT.FORWARD) #next command is not processed so better to inverse command sequence
        self._left.run(Adafruit_MotorHAT.BACKWARD)
        # If an amount of time is specified, move for that time and then stop.
        if seconds is not None:
            time.sleep(seconds)
